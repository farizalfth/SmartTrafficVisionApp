    ...
    if (_isInit) {
      ...
      if (dataProvider.cctvList.isNotEmpty) {
        if (widget.initialCCTVId != null) {
          try {
            _selectedCCTV = dataProvider.cctvList.firstWhere((c) => c.id == widget.initialCCTVId);
          } catch (e) {
    ...
    return Scaffold(
      ...
      body: Consumer<CCTVDataSource>(
        builder: (context, dataSource, child) {
          ...
          if (allCCTVs.isEmpty) return const Center(child: Text("Belum ada data CCTV."));

          final filteredList = allCCTVs.where((cctv) {
            final matchesSearch = cctv.name.toLowerCase().contains(_searchQuery.toLowerCase()) ||
          ...
          if (_selectedCCTV != null && !allCCTVs.any((c) => c.id == _selectedCCTV!.id)) {
          ...
          return SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                if (_selectedCCTV != null) _buildMainPlayerArea(context, _selectedCCTV!),
                ...
                if (filteredList.isEmpty && _searchQuery.isNotEmpty)
                  const Center(child: Padding(padding: EdgeInsets.all(20.0), child: Text("Tidak ditemukan CCTV yang cocok.", style: TextStyle(color: Colors.grey))))
                else
  ...
  Widget _buildMainPlayerArea(BuildContext context, CCTV cctv) {
    return Column(
      ...
      children: [
          ...
          child: _controller != null
              ...
              : Container(
                  ...
                  child: const Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                    ...
                    SizedBox(height: 10),
                    Text("Stream Offline / URL RTSP", style: TextStyle(color: Colors.white54)),
                  ])),
                ),
        ),
        const SizedBox(height: 16),
        ...
        Card(
          color: Theme.of(context).cardColor, elevation: 2, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
          child: Padding(
            padding: const EdgeInsets.all(16.0),
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(cctv.name, style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold)),
                const SizedBox(height: 4),
                Row(children: [
                  const Icon(Icons.location_on, color: Colors.blueAccent, size: 14),
                  const SizedBox(width: 4),
                  Expanded(child: Text(cctv.location, style: const TextStyle(color: Colors.white70), overflow: TextOverflow.ellipsis)),
                ]),
                const SizedBox(height: 12),
                
                // --- STREAM BUILDER KHUSUS ID CCTV INI ---
                StreamBuilder(
                  stream: _trafficStatsRef.child(cctv.id).onValue, // Mengambil data berdasarkan ID (misal: "3", "4")
                      ...
                      try {
                        ...
                        if (rawValue is Map) {
                          if (rawValue.containsKey('live')) {
                             ...
                             countText = liveData['total']?.toString() ?? "0";
                             densityText = liveData['status']?.toString() ?? "Normal";
                          } else {
                             // Struktur ID 3: { total: 2, status: "Lancar" }
                             countText = rawValue['total']?.toString() ?? "0";
                             densityText = rawValue['status']?.toString() ?? "Normal";
                          }
                        }
                        ...
                        if (densityText.toLowerCase().contains("macet")) {
                          status = "Macet";
                        } else if (densityText.toLowerCase().contains("padat")) status = "Padat";
                    ...
                    return Row(
                      ...
                      children: [
                        ...
                        Container(
                          ...
                          child: Text("Status: $status", style: TextStyle(color: statusColor, fontWeight: FontWeight.bold, fontSize: 12)),
                        ),
                        // DETAIL KENDARAAN
                        Column(
                          crossAxisAlignment: CrossAxisAlignment.end,
                          children: [
                            Text("Kepadatan: $densityText", style: TextStyle(fontSize: 12, color: status == "Macet" ? Colors.redAccent : Colors.white70)),
                            Text("Jumlah Kendaraan: $countText", style: const TextStyle(fontSize: 12, color: Colors.white, fontWeight: FontWeight.bold)),
                          ],
                        )
                      ],
                    );
    ...
    return GridView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return GestureDetector(
          ...
          child: Card(
            color: Theme.of(context).cardColor, elevation: 3, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Expanded(
                child: Container(
                  decoration: const BoxDecoration(borderRadius: BorderRadius.vertical(top: Radius.circular(12)), color: Colors.black26),
                  child: Center(
                    child: Container(
                      padding: const EdgeInsets.all(12),
                      decoration: BoxDecoration(color: Colors.red.withOpacity(0.9), shape: BoxShape.circle, boxShadow: [BoxShadow(color: Colors.red.withOpacity(0.4), blurRadius: 10, spreadRadius: 2)]),
              ...
              Padding(
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text(item.name, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 13, color: Colors.white), maxLines: 2, overflow: TextOverflow.ellipsis),
                  const SizedBox(height: 4),
                  Text(item.location, style: TextStyle(color: Colors.grey[400], fontSize: 11), maxLines: 1, overflow: TextOverflow.ellipsis),
                ]),
              ),
            ]),
          ),
